...
//         }
    ...
    public function testSendDeleteEmail() {
        ...
        if(!$b){
            ...
            $this->mailboxCleanUp($expected_subject);            
        }        
    ...
    private function mailboxCleanUp($expected_subject){
        ...
        if (!$conn) {
            ...
            self::$climate->info('Scanning recipient\'s mailbox: ' . getenv('MAIL_USER'));
            if (count($messages) > 0) {
                foreach ($messages as $msg) {
                    $subject = $msg['subject'];
                    $msg_num = $msg['msgno'];
                    self::$climate->info('Found message, subject is: ' . $subject);
                ...
                self::$climate->info('No messages found.');
            }
